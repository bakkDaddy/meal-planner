...
function renderMeals() {
  ...
  list.querySelectorAll('.meal-card').forEach(card => {
    card.addEventListener('click', (e) => {
      ...
      history.pushState({ view: 'cookSteps', mealId: card.dataset.meal }, '');
      showView('cook');
...
function renderCookSelect() {
  ...
  container.querySelectorAll('.cook-meal-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      history.pushState({ view: 'cookSteps', mealId: btn.dataset.meal }, '');
      renderCookSteps(btn.dataset.meal);
    });
  });
}
...
function renderCookSteps(mealId) {
  ...
  stepsContainer.querySelector('.cook-back').addEventListener('click', () => {
    history.back();
  });
...
document.querySelectorAll('.nav-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    history.pushState({ view: btn.dataset.view }, '');
...
window.addEventListener('popstate', (e) => {
  ...
  if (e.state.view === 'cookSteps' && e.state.mealId) {
    showView('cook');
    renderCookSteps(e.state.mealId);
  } else if (e.state.view) {
    showView(e.state.view);
    if (e.state.view === 'cook') renderCookSelect();
  } else {
    showView('meals');
  }
});
